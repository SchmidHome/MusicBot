...
export class ConsoleLogger implements Logger {
  ...
  public constructor(private name: string) {
    ...
    ConsoleLogger.maxNameLength = Math.max(
      ConsoleLogger.maxNameLength,
      name.length
    );
  ...
  private append(
    ...
    const msg = `[${this.getTime()}] [${colorType(type)}] [${this.color(
      padCenter(this.name, ConsoleLogger.maxNameLength)
    )}] ${colorText(message)}`;
